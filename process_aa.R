...
aa_compare <- function(input){
  ...
  aa_bands <- c("delta_abs","theta_abs","alpha_abs","beta_abs","delta_rel","theta_rel","alpha_rel","beta_rel")
...
#process completer, responder, non-responder and remission and tidy into data_frame

library('broom')
completer_aa <- aa_compare(completer)
completer_aa <- aa_compare(completer)
completer_aa <- aa_compare(completer)
completer_aa <- aa_compare(completer)
