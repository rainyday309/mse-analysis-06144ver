...
spectral <- read.csv('spectralOutput_debug.csv', header = TRUE, stringsAsFactors=FALSE, fileEncoding="UTF-8")
...
#debug: calculate relative power of alpha, beta, theta
...
# take logarithm of 10 for absolute power of spectral analysis
completer[,grep(patter='abs',names(completer))] = log(completer[,grep(patter='abs',names(completer))],10)
...
# get subgroups: remission, responder, non-responder
...
non_remission <- completer[!(completer$HAMD6 < 7),]
